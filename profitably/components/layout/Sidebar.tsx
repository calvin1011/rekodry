    ...
    <>
      ...
      <div className={`md:hidden fixed inset-0 z-50 ${isMobileOpen ? '' : 'pointer-events-none'}`}>
        ...
        >
          ...
          <nav className="px-4 py-4 overflow-y-auto custom-scrollbar h-[calc(100%-120px)]">
            {navSections.map((section) => (
              <div key={section.title} className="mb-6">
                <p className="px-4 mb-2 text-xs font-semibold text-slate-500 uppercase tracking-wider">{section.title}</p>
                  ...
                  {section.items.map((item) => (
                    <Link
                      ...
                      className={`flex items-center gap-3 px-4 py-2.5 rounded-xl transition-all
                        ${isActive(item.href)
                          ? 'bg-profit-500/15 text-profit-400 shadow-sm'
                          : 'text-slate-400 hover:bg-slate-800/50 hover:text-slate-100'
                        }`}
                    >
                      <span className={isActive(item.href) ? 'text-profit-400' : ''}>{item.icon}</span>
      ...
      <aside className="hidden md:flex md:flex-col md:w-64 bg-slate-900/50 border-r border-slate-800/50 min-h-screen">
        <div className="p-5 border-b border-slate-800/50">
          <Link href="/dashboard" className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-profit-500 to-profit-600 flex items-center justify-center shadow-lg shadow-profit-500/20">
              <span className="text-white font-bold text-xl">R</span>
            </div>
            <span className="text-xl font-bold text-slate-100">Rekodry</span>
          </Link>
        </div>
        ...
        <nav className="flex-1 px-3 py-4 overflow-y-auto custom-scrollbar">
          {navSections.map((section) => (
            <div key={section.title} className="mb-6">
              <p className="px-3 mb-2 text-xs font-semibold text-slate-500 uppercase tracking-wider">{section.title}</p>
              <div className="space-y-1">
                {section.items.map((item) => (
                  <Link
                    key={item.href}
                    ...
                    className={`flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all group
                      ${isActive(item.href)
                        ? 'bg-profit-500/15 text-profit-400'
                        : 'text-slate-400 hover:bg-slate-800/50 hover:text-slate-100'
                      }`}
                  >
                    <span className={`transition-colors ${isActive(item.href) ? 'text-profit-400' : 'group-hover:text-slate-200'}`}>
                      {item.icon}
                    </span>
                    <span className="font-medium text-sm">{item.name}</span>
                    {isActive(item.href) && (
                      <div className="ml-auto w-1.5 h-1.5 rounded-full bg-profit-400" />
                    )}
                  </Link>
                ))}
              </div>
